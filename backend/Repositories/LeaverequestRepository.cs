...
namespace backend.Repositories
{
  public class LeaverequestRepository
  {
    ...
    public async Task<Leaverequest> GetLeaverequestByStartDateAsync(DateOnly startDate, int employeeId)
    {
      return await _manageEmployeeDbContext.Leaverequests.FirstOrDefaultAsync(
          x => x.EmployeeId == employeeId && x.StartDate == startDate
      );
    }

    public async Task<Leaverequest> GetLeaverequestByEndDateAsync(DateOnly endDate, int employeeId)
    {
      return await _manageEmployeeDbContext.Leaverequests.FirstOrDefaultAsync(
          x => x.EmployeeId == employeeId && x.EndDate == endDate
      );
    }
